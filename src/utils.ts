...
import { Episode, StreamLink } from './types.js';

const rl = createInterface({ input, output });

export const ask = async (question: string) => (await rl.question(question)).trim();

export const commandExists = (command: string) => new Promise<boolean>((resolve) => {
...
export const tryExternalMenu = async <T>(
  title: string,
  items: T[],
...
export const chooseFromList = async <T>(
  title: string,
  items: T[],
  render: (item: T, index: number) => string,
): Promise<T> => {
...
export const parseEpisodeRange = (range: string, episodes: Episode[]) => {
...
export const installFfmpegWithWinget = async (yes: boolean) => {
...
export const requireFfmpegCommand = async (yes: boolean) => {
...
export const getFfmpegHlsExtensionArgs = (ffmpeg: string) => {
...
export const probeHlsDurationMs = async (url: string, referer: string) => {
  try {
...
export const probeDurationMs = async (ffmpeg: string, url: string, referer: string, hlsExtensionArgs: string[]) => {
  if (/\.m3u8(?:[?#]|$)/i.test(url)) {
...
export const normalizeAudio = (value: unknown) => {
  const lower = String(value || '').toLowerCase();
  if (/(dub|eng|english)/.test(lower)) return 'dub';
  return 'sub';
};

export const scoreStream = (stream: StreamLink) => {
